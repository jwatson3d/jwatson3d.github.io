...
CLASS FileSample
    LOOP until exit requested
        ...
        IF option chosen is character I/O
            PERFORM CharacterIO.ReadFile
        ELSE IF option chosen is buffered I/O
            PERFORM BufferedIO.ReadFile
...
There are three classes involved - the main console application which is `FileSample`, a class to handle buffered I/O called `BufferedIO` and a class to handle character I/O called `CharacterIO`. Both of the `IO` classes have a method called `ReadFile` containing the logic necessary to read and display a text file. Even from just this high-level pseudocode you can see that using the buffered I/O approach is much simpler from a programmer's point of view - read a line, split that line into fields and print them. That's a whopping three lines of effort. Compare that to character I/O where each character is read one-at-a-time and appended to a string variable, when a comma is reached (or whatever delimiter is used in the file) then a field number variable is incremented so that you are building up the correct field. This lower-level type of reading requires ten lines of effort using character I/O to accomplish the same thing that three lines of buffered I/O can do.
...
public class FileSample {
    ...
    public static void main(String[] args) {
        ...
        while (!exit) {
            ...
            switch (option) {
                case 1:
                    CharacterIO characterIO = new CharacterIO(filename);
                    characterIO.readFile();
                    break;
                case 2:
                    BufferedIO bufferedIO = new BufferedIO(filename);
                    bufferedIO.readFile();
                    break;
                case 9:
                    exit = true;
...
There are three separate tasks that the method accomplishes:
...
3. Use the `File` class instance variable just initialized in step #2 to create a `FileReader` class.
...
The next two actions (creating a `File` and `FileReader`) causes the program to attempt to access the file on the hard drive which leads to the possibility of errors, more formally known as exceptions, so they are contained within `try-catch` blocks. Doing so allows a meaningful error message to be displayed and the `prepareFile()` method can immediatly return `false`. The `try-catch` technique avoids the program crashing and displaying an ugly stack-trace message that users would not be expected to make sense of.
...
## CharacterIO
Essentially the `CharacterIO` class is identical to the `BufferedIO` above with the difference of the `readFile()` method.

### prepareFile()
The `prepareFile()` method is identical to the one already shown in `BufferedIO`.
